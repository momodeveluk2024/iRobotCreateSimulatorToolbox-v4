...
title(handles.axes_map, '');

% ============================================================
% ENHANCED UI
% Beautiful, clean, modern interface with icons and descriptions
% ============================================================

% Set figure properties for a stable, resizable pixel layout
set(hObject, 'Color', [0.96 0.97 0.98], 'Resize', 'on', 'Units', 'pixels');

if isfield(handles, 'push_wall_line')
    set(handles.push_wall_line, 'String', '<html><span style="font-size: 18px; color: #333;">&#9586;</span></html>', ...
        'FontSize', 16, 'FontWeight', 'bold', ...
        'TooltipString', 'Single Line - click endpoints', ...
        'BackgroundColor', [0.98 0.98 1.0]);
end
if isfield(handles, 'push_wall_cont')
    set(handles.push_wall_cont, 'String', '<html><span style="font-size: 18px; color: #333;">&#8780;</span></html>', ...
        'FontSize', 14, 'FontWeight', 'bold', ...
        'TooltipString', 'Continuous Line - press Enter to break string', ...
        'BackgroundColor', [0.98 0.98 1.0]);
end
if isfield(handles, 'push_wall_square')
    set(handles.push_wall_square, 'String', '<html><span style="font-size: 18px; color: #333;">&#9723;</span></html>', ...
        'FontSize', 16, 'FontWeight', 'bold', ...
        'TooltipString', 'Square - click diagonal corners', ...
        'BackgroundColor', [0.98 0.98 1.0]);
end

% ============================================================
% LINE SECTION - Dashed style icons
% ============================================================
if isfield(handles, 'push_line_line')
    set(handles.push_line_line, 'String', '<html><span style="font-size: 14px; color: #333;">&#10515;</span></html>', ...
        'FontSize', 10, 'FontWeight', 'bold', ...
        'TooltipString', 'Single Dashed Line - click endpoints', ...
        'BackgroundColor', [0.98 0.98 1.0]);
end
if isfield(handles, 'push_line_cont')
    set(handles.push_line_cont, 'String', '<html><span style="font-size: 18px; color: #333;">&#10510;</span></html>', ...
        'FontSize', 14, 'FontWeight', 'bold', ...
        'TooltipString', 'Continuous Dashed Line - press Enter to break', ...
        'BackgroundColor', [0.98 0.98 1.0]);
end
if isfield(handles, 'push_line_square')
    set(handles.push_line_square, 'String', '<html><span style="font-size: 18px; color: #333;">&#9634;</span></html>', ...
        'FontSize', 16, 'FontWeight', 'bold', ...
        'TooltipString', 'Dashed Square - click diagonal corners', ...
        'BackgroundColor', [0.98 0.98 1.0]);
end

% ============================================================
% BEACON SECTION - Colored icon
% ============================================================
if isfield(handles, 'push_beacon')
    set(handles.push_beacon, 'String', '<html><span style="font-size: 18px; color: #D00;">&#11044;</span></html>', ...
        'FontSize', 16, 'FontWeight', 'bold', ...
        'ForegroundColor', [0.7 0 0], ...
        'TooltipString', 'Place Beacon - select color and ID first', ...
        'BackgroundColor', [0.98 0.98 1.0]);
end

% ============================================================
% VIRTUAL WALL SECTION - Green triangle
% ============================================================
if isfield(handles, 'push_vwall')
    set(handles.push_vwall, 'String', '<html><span style="font-size: 16px; color: #090;">&#9654;</span></html>', ...
        'FontSize', 14, 'FontWeight', 'bold', ...
        'ForegroundColor', [0 0.6 0.2], ...
        'TooltipString', 'Virtual Wall - click emitter point then direction', ...
        'BackgroundColor', [0.98 0.98 1.0]);
end

% ============================================================
% ACTION BUTTONS - Modern colored style
% ============================================================
if isfield(handles, 'push_undo')
    set(handles.push_undo, 'BackgroundColor', [0.93 0.93 0.96], ...
        'FontWeight', 'bold');
end
if isfield(handles, 'push_clear')
    set(handles.push_clear, 'BackgroundColor', [0.93 0.93 0.96], ...
        'FontWeight', 'bold');
end
if isfield(handles, 'push_load')
    set(handles.push_load, 'BackgroundColor', [0.7 0.85 0.95], ...
        'FontWeight', 'bold', 'ForegroundColor', [0.1 0.3 0.5]);
end
if isfield(handles, 'push_save')
    set(handles.push_save, 'BackgroundColor', [0.75 0.92 0.75], ...
        'FontWeight', 'bold', 'ForegroundColor', [0.1 0.4 0.1]);
end

% ============================================================
% STYLE ALL PANELS for consistent modern look
% ============================================================
allPanels = findall(hObject, 'Type', 'uipanel');
for i = 1:length(allPanels)
    set(allPanels(i), 'BackgroundColor', [0.96 0.97 0.98], ...
        'ForegroundColor', [0.25 0.3 0.4], ...
        'FontWeight', 'bold', 'FontSize', 9);
end

% Style radio buttons
allRadios = findall(hObject, 'Style', 'radiobutton');
for i = 1:length(allRadios)
    set(allRadios(i), 'BackgroundColor', [0.96 0.97 0.98]);
end

% Style text labels
allTexts = findall(hObject, 'Style', 'text');
for i = 1:length(allTexts)
    currentBg = get(allTexts(i), 'BackgroundColor');
    if ~isequal(currentBg, [1 1 1])  % Don't change white backgrounds
        set(allTexts(i), 'BackgroundColor', [0.96 0.97 0.98]);
    end
end

    % Clean up figure units
    set(hObject, 'Resize', 'on', 'Units', 'pixels');
    drawnow; 
    
    % Strategy 2: Identification by Exclusion
    % We know the handles of all 'named' buttons in 'handles' struct. 
    % Any other button is likely a Shapes button.
    
    knownHandles = [];
    fNames = fieldnames(handles);
    for i = 1:length(fNames)
        fVal = handles.(fNames{i});
        if isgraphics(fVal) && strcmp(get(fVal, 'Type'), 'uicontrol')
            knownHandles = [knownHandles; double(fVal)]; % Store as handle
        end
    end
    
    allButtons = findall(hObject, 'Style', 'pushbutton');
    foundButtons = [];
    foundY = [];
    
    % fprintf('--- Button Identification Log ---\n');
    for k = 1:length(allButtons)
        btn = allButtons(k);
        tag = get(btn, 'Tag');
        
        % Check by exclusion
        isKnownType = contains(tag, 'wall') || contains(tag, 'line') || ...
                      contains(tag, 'beacon') || contains(tag, 'undo') || ...
                      contains(tag, 'clear') || contains(tag, 'load') || ...
                      contains(tag, 'save');
                      
        if ~isKnownType
            % This is a candidate for Shapes button
            bPos = getpixelposition(btn);
            foundButtons = [foundButtons; btn];
            foundY = [foundY; bPos(2)];
        end
    end
    
    % Sort buttons from Top to Bottom
    if ~isempty(foundButtons)
        [~, idx] = sort(foundY, 'descend');
        foundButtons = foundButtons(idx);
        
        % Take the top 3 buttons found (assuming there are 3)
        loopMax = min(3, length(foundButtons));
        
        for j = 1:loopMax
            btn = foundButtons(j);
            
            % Style button
            set(btn, 'BackgroundColor', [0.98 0.98 1.0], ...
                'FontSize', 14, 'FontWeight', 'bold', ...
                'ForegroundColor', [0 0 0]); % Black text
            
            % Determine content based on order
            desc = '';
            if j == 1
                set(btn, 'String', '<html><span style="font-size: 18px; color: #333;">&#9586;</span></html>'); % Line
                desc = 'Single Line';
            elseif j == 2
                set(btn, 'String', '<html><span style="font-size: 18px; color: #333;">&#8780;</span></html>'); % Chevron
                desc = 'Continuous Line';
            else
                set(btn, 'String', '<html><span style="font-size: 18px; color: #333;">&#9723;</span></html>'); % Square
                desc = 'Square';
            end
            
            % Set tooltip for good measure
            set(btn, 'TooltipString', desc);
            
            % Create label (Position relative to button)
            thisBPos = getpixelposition(btn);
            textX = thisBPos(1) + thisBPos(3) + 5;
            textY = thisBPos(2);
            
            % Parent the text to the SAME parent as the button
            % This ensures (x,y) coordinates from getpixelposition(btn) match
            btnParent = get(btn, 'Parent');
            
            uicontrol('Parent', btnParent, ... 
                'Style', 'text', ...
                'String', desc, ...
                'Units', 'pixels', ...
                'Position', [textX, textY, 150, 20], ...
                'BackgroundColor', [0.96 0.97 0.98], ...
                'HorizontalAlignment', 'left', ...
                'FontSize', 8, ...
                'ForegroundColor', [0.3 0.3 0.4]);
        end
    end
    layoutMapMakerGUI(hObject, handles);
    set(hObject, 'ResizeFcn', @(src,evt) layoutMapMakerGUI(src, guidata(src)));
...
function cleanupTimer(hObject, ~)
    delete(hObject);



function showMapMakerStatus(handles, ~)
if isfield(handles, 'axes_map') && ishandle(handles.axes_map)
    title(handles.axes_map, '');
end

function layoutMapMakerGUI(fig, handles)
if isempty(handles) || ~ishandle(fig)
    return
end
...
if figPos(3) < minWidth || figPos(4) < minHeight
    ...
    figPos(4) = max(figPos(4), minHeight);
...
gap = 24;
rightWidth = 260;
sidebarX = width - margin - rightWidth;
axesX = 40;
availableW = max(420, sidebarX - gap - axesX);
availableH = max(420, height - 210);
axesSize = min(availableW, availableH);
axesY = max(100, height - 100 - axesSize);

layoutHandle(findall(fig, 'Tag', 'axes_map'), [axesX axesY axesSize axesSize]);
layoutHandle(findall(fig, 'Tag', 'text1'), [axesX + axesSize/2 - 120 height - 70 240 38]);

topY = height - 200;
layoutHandle(findall(fig, 'Tag', 'uipanel1'), [sidebarX topY - 150 rightWidth 150]);
layoutHandle(findall(fig, 'Tag', 'uipanel2'), [sidebarX topY - 230 rightWidth 70]);
layoutHandle(findall(fig, 'Tag', 'uipanel3'), [sidebarX topY - 310 rightWidth 70]);
layoutHandle(findall(fig, 'Tag', 'uipanel4'), [sidebarX topY - 410 rightWidth 90]);
layoutHandle(findall(fig, 'Tag', 'uipanel5'), [sidebarX topY - 510 rightWidth 90]);

layoutHandle(findall(fig, 'Tag', 'text3'), [sidebarX height - 82 rightWidth 24]);
layoutHandle(findall(fig, 'Tag', 'text4'), [sidebarX height - 128 rightWidth 24]);
layoutHandle(findall(fig, 'Tag', 'text6'), [sidebarX height - 174 rightWidth 24]);

buttonGap = 12;
buttonW = min(108, (axesSize - 3*buttonGap)/4);
buttonH = 46;
buttonY = 24;
buttonX = axesX;
layoutHandle(findall(fig, 'Tag', 'push_undo'), [buttonX buttonY buttonW buttonH]);
layoutHandle(findall(fig, 'Tag', 'push_clear'), [buttonX + buttonW + buttonGap buttonY buttonW buttonH]);
layoutHandle(findall(fig, 'Tag', 'push_load'), [buttonX + 2*(buttonW + buttonGap) buttonY buttonW buttonH]);
layoutHandle(findall(fig, 'Tag', 'push_save'), [buttonX + 3*(buttonW + buttonGap) buttonY buttonW buttonH]);
drawnow

function layoutHandle(handleList, position)
if ~isempty(handleList) && ishandle(handleList(1))
    set(handleList(1), 'Units', 'pixels', 'Position', position);
end
...
if ~isempty(x) && length(x) == 2    % Check that two points were clicked
    ...
    if isPointInAxesBounds(x, y, xLimits, yLimits)
        mapMat= [mapMat ; ...
        ...
        showMapMakerStatus(handles, 'Point out of bounds - action cancelled');
    end
end
...
while ~isempty(button) && button >= 1 && button <= 3
    ...
    if ~isempty(button) && button >= 1 && button <= 3
        xLimits= xlim(handles.axes_map);
        yLimits= ylim(handles.axes_map);
        if isPointInAxesBounds([x1 x2], [y1 y2], xLimits, yLimits)
            mapMat= [mapMat ; ...
            ...
            showMapMakerStatus(handles, 'Point out of bounds - finished');
            break; % Exit loop
        end
    end
end
...
if ~isempty(x) && length(x) == 2    % Check that two points were clicked
    xLimits= xlim(handles.axes_map);
    yLimits= ylim(handles.axes_map);
    if isPointInAxesBounds(x(1), y(1), xLimits, yLimits)
