...
%                     [y_sensor y_sensor+dist*sin(th_sensor)])
        ...
        %   travelDist(0.200,1.500) - input argument functions
            if isempty(obj.dataHist)
                if isempty(obj.timeElap)
                    timeNow = 0;
                else
                    timeNow = toc(obj.timeElap);
                end
                obj.dataHist = {timeNow obj.posAbs(1) obj.posAbs(2) obj.thAbs []};
            elseif size(obj.dataHist,2) < 5
                obj.dataHist{end,5} = [];
            end
