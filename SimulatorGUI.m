...
set(hObject, 'Color', [0.94 0.94 0.96], 'Resize', 'on', 'Units', 'pixels');

% Navigation/Setup buttons with enhanced styling
if isfield(handles, 'push_map')
    set(handles.push_map, 'String', 'Load Map', ...
        'FontSize', 9, 'FontWeight', 'bold', ...
        'TooltipString', 'Import a map file (.txt)', ...
        'BackgroundColor', [0.85 0.9 0.95]);
end
if isfield(handles, 'push_config')
    set(handles.push_config, 'String', 'Config', ...
        'FontSize', 9, 'FontWeight', 'bold', ...
        'TooltipString', 'Load robot configuration file', ...
        'BackgroundColor', [0.9 0.9 0.95]);
end
if isfield(handles, 'push_origin')
    set(handles.push_origin, 'String', 'Origin', ...
        'FontSize', 9, 'FontWeight', 'bold', ...
        'TooltipString', 'Set robot starting position and direction', ...
        'BackgroundColor', [0.9 0.9 0.95]);
end
if isfield(handles, 'push_sensors')
    set(handles.push_sensors, 'String', 'Sensors', ...
        'FontSize', 9, 'FontWeight', 'bold', ...
        'TooltipString', 'Query all sensor readings', ...
        'BackgroundColor', [0.9 0.95 0.9]);
end

% Manual control buttons - Arrow keys with modern styling
if isfield(handles, 'push_fwd')
    set(handles.push_fwd, 'String', '<html><span style="font-size: 24px; color: #333333;">&#8593;</span></html>', ...  % Up arrow
        'FontSize', 14, 'FontWeight', 'bold', ...
        'TooltipString', 'Move Forward (Up Arrow or W)', ...
        'BackgroundColor', [0.85 0.95 0.85]);
end
if isfield(handles, 'push_back')
    set(handles.push_back, 'String', '<html><span style="font-size: 24px; color: #333333;">&#8595;</span></html>', ...  % Down arrow
        'FontSize', 14, 'FontWeight', 'bold', ...
        'TooltipString', 'Move Backward (Down Arrow or S)', ...
        'BackgroundColor', [0.95 0.85 0.85]);
end
if isfield(handles, 'push_left')
    set(handles.push_left, 'String', '<html><span style="font-size: 24px; color: #333333;">&#8592;</span></html>', ...  % Left arrow
        'FontSize', 14, 'FontWeight', 'bold', ...
        'TooltipString', 'Turn Left (Left Arrow or A)', ...
        'BackgroundColor', [0.85 0.85 0.95]);
end
if isfield(handles, 'push_right')
    set(handles.push_right, 'String', '<html><span style="font-size: 24px; color: #333333;">&#8594;</span></html>', ...  % Right arrow
        'FontSize', 14, 'FontWeight', 'bold', ...
        'TooltipString', 'Turn Right (Right Arrow or D)', ...
        'BackgroundColor', [0.85 0.85 0.95]);
end
if isfield(handles, 'push_stop')
    set(handles.push_stop, 'String', 'STOP', ...
        'FontSize', 10, 'FontWeight', 'bold', ...
        'ForegroundColor', [0.8 0 0], ...  % Red text
        'TooltipString', 'Emergency Stop (Space/Enter/Esc)', ...
        'BackgroundColor', [1 0.9 0.9]);
end

% Autonomous control buttons
if isfield(handles, 'push_auto_start')
    set(handles.push_auto_start, 'String', 'Start Auto', ...
        'FontSize', 9, 'FontWeight', 'bold', ...
        'ForegroundColor', [0 0.5 0], ...  % Green text
        'TooltipString', 'Start autonomous control program', ...
        'BackgroundColor', [0.85 0.95 0.85]);
end
if isfield(handles, 'push_auto_stop')
    set(handles.push_auto_stop, 'String', 'Stop Auto', ...
        'FontSize', 9, 'FontWeight', 'bold', ...
        'ForegroundColor', [0.7 0 0], ...  % Red text
        'TooltipString', 'Stop autonomous control', ...
        'BackgroundColor', [0.95 0.85 0.85]);
end



handles = createSpeedControl(handles);
layoutSimulatorGUI(hObject, handles);
set(hObject, 'ResizeFcn', @(src,evt) layoutSimulatorGUI(src, guidata(src)));
...
function handles = createSpeedControl(handles)
if ~isfield(handles, 'figure_simulator') || ~ishandle(handles.figure_simulator)
    return
end

speedPanel = findall(handles.figure_simulator, 'Tag', 'uipanel6');
if isempty(speedPanel) || ~ishandle(speedPanel(1))
    return
end
speedPanel = speedPanel(1);

oldRadios = [];
for tag = {'radio_speed1', 'radio_speed2', 'radio_speed3'}
    if isfield(handles, tag{1}) && ishandle(handles.(tag{1}))
        oldRadios = [oldRadios handles.(tag{1})];
    end
end
if ~isempty(oldRadios)
    set(oldRadios, 'Visible', 'off');
end

popupSpeed = findall(speedPanel, 'Tag', 'popup_speed');
if isempty(popupSpeed) || ~ishandle(popupSpeed(1))
    popupSpeed = uicontrol('Parent', speedPanel, ...
        'Style', 'popupmenu', ...
        'Tag', 'popup_speed', ...
        'String', {'1x','2x','3x','4x','5x','6x','7x','8x','9x','10x'}, ...
        'Value', 1, ...
        'Units', 'pixels', ...
        'Position', [12 20 116 24], ...
        'BackgroundColor', [1 1 1], ...
        'TooltipString', 'Simulation speed multiplier');
else
    popupSpeed = popupSpeed(1);
end
handles.popup_speed = popupSpeed;

function layoutSimulatorGUI(fig, handles)
...
axesX = margin;
axesY = 38;
axesW = max(420, sidebarX - gap - axesX);
axesH = max(380, height - topHeight - axesY - 18);

layoutHandle(findall(fig, 'Tag', 'axes_map'), [axesX axesY axesW axesH]);
layoutHandle(findall(fig, 'Tag', 'uipanel5'), [margin height - 76 500 56]);
layoutHandle(findall(fig, 'Tag', 'text_title'), [axesX + axesW/2 - 150 height - 70 300 40]);
layoutHandle(findall(fig, 'Tag', 'push_map'), [sidebarX height - 76 rightWidth 46]);
layoutHandle(findall(fig, 'Tag', 'push_config'), [sidebarX height - 132 rightWidth 46]);
layoutHandle(findall(fig, 'Tag', 'push_origin'), [sidebarX height - 188 rightWidth 46]);
layoutHandle(findall(fig, 'Tag', 'text7'), [sidebarX height - 214 rightWidth 20]);

panelY = height - 270;
layoutHandle(findall(fig, 'Tag', 'uipanel3'), [sidebarX panelY rightWidth 52]);
panelY = panelY - 188;
layoutHandle(findall(fig, 'Tag', 'uipanel1'), [sidebarX panelY rightWidth 178]);
panelY = panelY - 170;
layoutHandle(findall(fig, 'Tag', 'uipanel2'), [sidebarX panelY rightWidth 158]);
panelY = panelY - 104;
layoutHandle(findall(fig, 'Tag', 'uipanel_auto'), [sidebarX panelY rightWidth 94]);
panelY = panelY - 72;
layoutHandle(findall(fig, 'Tag', 'uipanel6'), [sidebarX panelY rightWidth 62]);

if isfield(handles, 'popup_speed') && ishandle(handles.popup_speed)
    set(handles.popup_speed, 'Units', 'pixels', ...
        'Position', [12 20 rightWidth - 24 24]);
end
